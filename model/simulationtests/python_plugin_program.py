...
class AverageZoneTemps(EnergyPlusPlugin):
    ...
    def on_end_of_zone_timestep_before_zone_reporting(self, state) -> int:
        if self.do_setup:
            ...
            zone_names = [
                "Story 1 Core Thermal Zone",
                "Story 1 East Perimeter Thermal Zone",
                "Story 1 North Perimeter Thermal Zone",
                "Story 1 South Perimeter Thermal Zone",
                "Story 1 West Perimeter Thermal Zone",
            ]
            ...
            self.data["running_avg_temp_variable"] = self.api.exchange.get_global_handle(
                state, "RunningAverageBuildingTemp"
            )
            self.do_setup = False
        zone_temps = list()
